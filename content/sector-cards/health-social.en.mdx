---
title: "Health COCOM: Samusocial Nozat, Humanitarian Hub evicted, 23% poverty, early childhood lasagna"
seoTitle: "Health and social care in Brussels: state of the sector"
slug: health-social
locale: en
parentDomain: social
sector: "Health and bicommunautary social affairs (COCOM)"
frozenMechanisms: []
activeMechanisms:
  - name: "Iriscare — routine operations"
    description: "Iriscare ensures provider payments, management of existing approvals, inspections and individual case handling."
  - name: "Bruss'Help — count and coordination"
    description: "Bruss'Help continues its periodic counts of homeless people and operational coordination for the sector."
  - name: "Winter shelter scheme"
    description: "The winter emergency shelter scheme is maintained each year."
  - name: "e-Health Plan 2026-2029"
    description: "The federal Public Health department published the e-Health 2026-2029 action plan, applicable to Brussels."
impactIndicators:
  - label: "Homeless or roofless people"
    value: "9,777"
    source: "Bruss'Help (8th count, November 2024)"
    url: "https://www.brusshelp.org/publications"
    frequency: "annuel"
  - label: "At-risk-of-poverty rate (AROP, 2025)"
    value: "23% (vs 28% in 2023)"
    source: "Health and Social Observatory / SILC"
    url: "https://www.ccc-ggc.brussels/fr/observatoire-de-la-sante-et-du-social"
    frequency: "annuel"
  - label: "Risk of poverty or social exclusion (AROPE, 2025)"
    value: "33.6% (vs 16.5% Belgium)"
    source: "Statbel (EU-SILC 2025)"
    url: "https://statbel.fgov.be/fr/themes/menages/pauvrete-et-conditions-de-vie/risque-de-pauvrete-ou-dexclusion-sociale"
    frequency: "annuel"
  - label: "Integration income (CPAS) recipients in Brussels"
    value: "47,304"
    source: "Health and Social Observatory"
    url: "https://www.ccc-ggc.brussels/fr/observatoire-de-la-sante-et-du-social"
    frequency: "annuel"
  - label: "Students on integration income in Brussels (record)"
    value: "14,246 (vs 10,064 in Flanders)"
    source: "Health and Social Observatory (April 2026)"
    url: "https://www.ccc-ggc.brussels/fr/observatoire-de-la-sante-et-du-social"
    frequency: "annuel"
  - label: "Convalescence — call for projects (beds)"
    value: "130 beds targeted"
    source: "Iriscare / call for projects 1 April 2026 (closure 15 May)"
    url: "https://www.iriscare.brussels/"
    frequency: "ponctuel"
  - label: "Children receiving family allowances"
    value: "308,000+"
    source: "Famiris / COCOM"
    url: "https://famiris.brussels/"
    frequency: "annuel"
  - label: "INAMI healthcare budget (2026)"
    value: "€41.3bn"
    source: "INAMI / Le Spécialiste (2026)"
    frequency: "annuel"
  - label: "European Disability Cards issued (Brussels, 2025)"
    value: "5,871 (cumulative 14,490)"
    source: "Iriscare (February 2026)"
    url: "https://www.iriscare.brussels/"
    frequency: "annuel"
  - label: "Mobility aids (Iriscare expenditure 2024)"
    value: "€5.86M"
    source: "Iriscare"
    url: "https://stat.iriscare.brussels/comment-iriscare-soutient-la-mobilite-des-bruxellois/"
    frequency: "annuel"
  - label: "STIB Taxibus — end of PRM minibus service"
    value: "End 2027 (transition to private taxis)"
    source: "Van den Brandt, Brussels Parliament (17 March 2026)"
    url: "https://www.lalibre.be/belgique/mobilite/2026/03/17/elke-van-den-brandt-douche-les-espoirs-des-personnes-a-mobilite-reduite-a-bruxelles-KW42PQG5HZCRHKJ36V6K35TPYU/"
    frequency: "ponctuel"
stakeholders:
  - name: "Iriscare"
    type: agency
    url: "https://www.iriscare.brussels/"
  - name: "Bruss'Help"
    type: ngo
    url: "https://www.brusshelp.org/"
  - name: "Samusocial"
    type: ngo
    url: "https://samusocial.be/"
  - name: "Brussels-Capital Health and Social Observatory"
    type: agency
    url: "https://www.ccc-ggc.brussels/fr/observatoire-de-la-sante-et-du-social"
  - name: "Famiris"
    type: agency
    url: "https://famiris.brussels/"
humanImpact: "Samusocial has authorised alcohol in its women's centre since 20 April (pilot 'Nozat') against the 'infantilising' strict prohibition. The Humanitarian Hub (Avenue du Port) was evicted on 21 April. The at-risk-of-poverty rate remains at 23%, 33.6% of Brussels residents face poverty or social exclusion, and 14,246 Brussels students are on integration income — more than all of Flanders. Early childcare runs through an ONE / Opgroeien / Iriscare lasagna with FWB cuts (€74M)."
lastModified: "2026-09-22"
changeType: corrected
changeSummary: "Indicator precision: the 23% measures the at-risk-of-MONETARY-poverty rate (AROP, income below 60% of the national median), not the risk of poverty or social exclusion. This card said 'poverty rate' without specifying. The broader indicator (AROPE), which adds severe material and social deprivation and very low work intensity, reaches 33.6% in Brussels against 16.5% in Belgium (Statbel, EU-SILC 2025); it now appears on the card."
changeSummaryDate: "2026-09-22"
summaryFalc: "Samusocial allows some alcohol in the women's shelter since 20 April 2026. The Humanitarian Hub was evicted on 21 April. In Brussels, one in four people is at risk of poverty. Childcare places are missing."
---

## Samusocial — 'Nozat' pilot project (20 April 2026)

On **20 April 2026**, Samusocial launches the pilot project **'Nozat'** at the women's shelter: **beer and wine allowed in the ground-floor common areas** (locked fridge, **forbidden in the rooms**). An additional support worker is present for **3 months** to assess the scheme. Rationale by **Sarah de Liamchine** (general director): strict prohibition could be 'experienced as infantilising'. Goals: **harm reduction** (avoiding rushed consumption before curfew) and a more human welcome.

No change in shelter capacity or funding.

Source: L'Avenir, 'Beer and wine now allowed in Samusocial women's shelter' (20 April 2026).

## Humanitarian Hub — eviction of tents (21 April 2026)

The **City of Brussels** evicted, on **Tuesday 21 April 2026** in the morning, the **tents** set up next to the Humanitarian Hub (Avenue du Port), in coordination with **Bruss'Help**. Decision called 'difficult' by the City, in a context of chronic emergency-shelter saturation and ongoing migration pressure.

Source: [La Libre Bruxelles](https://www.lalibre.be/regions/bruxelles/2026/04/21/les-tentes-du-hub-humanitaire-evacuees-a-bruxelles-une-decision-difficile-pour-la-ville-NXUZJKYMQVHZZPZEDHQKFNBPQ4/) (21 April 2026).

## Poverty in Brussels: 23% at risk of monetary poverty and record student integration-income recipients

The Health and Social Observatory documents:

- **At-risk-of-poverty rate (AROP) 2025**: **23%** of the population, meaning an income below 60% of the national median (vs 28% in 2023 — the statistical gap is to be interpreted with caution)
- **Risk of poverty or social exclusion (AROPE) 2025**: **33.6%**, a broader indicator adding severe material and social deprivation and very low work intensity, against 16.5% for Belgium
- **47,304 integration-income (RIS) recipients** in Brussels
- **14,246 students on integration income** — i.e. **more than all of Flanders (10,064)** despite a five-times-smaller population. Wallonia: ~16,000.
- Rising trend among students: +14% over 5 years in Flanders, even stronger in Brussels

This concentration questions the **student funding model** (CPAS as the last safety net) and the link with regional / Community grants.

Source: Health and Social Observatory; Le Soir / RTBF (April 2026).

## Early childcare: the triple lasagna (ONE / Opgroeien / Iriscare)

Brussels early childcare falls simultaneously under **three authorities**:

- **ONE** (Office de la Naissance et de l'Enfance, FWB) — French-language accredited childcare
- **Opgroeien** (Flemish Community) — Dutch-language childcare
- **Iriscare** (COCOM) — bicommunautary coordination and certain structures

Concrete consequences:

- **Up to 10,000 places** missing in Brussels (Ligue des Familles)
- For 2 in 3 parents, finding a place is 'difficult or very difficult'
- **FWB cuts 2026**: **€74M** in early childcare (ONE, childcare, host homes)
- **Stork Plan**: 1,700 new places in Brussels + 1,800 in Wallonia (3,500 total) — but opening one place takes **7 to 9 years**
- **Opgroeien** plans ~850 new places via the Masterplan Kinderopvang (2025)

See the cross-cutting dossier: [Early childhood](/en/dossiers/petite-enfance).

Sources: Moustique (January 2026), BruxellesToday, RTBF, Opgroeien, BGM early-childhood dossier.

## Convalescence: 130 beds — Iriscare call for projects (1 April 2026)

Iriscare launched a **call for projects on 1 April 2026** (closure **15 May**) for the creation of **130 convalescence beds** in the Brussels Region. Objective: bridge the gap between hospital care and return home for fragile patients, particularly the elderly.

Source: Iriscare (April 2026).

## People with disabilities

- **~23,100** Brussels residents receive disability allowances (ARR/AI) — 9.1% of the national total
- **5,871** European Disability Cards issued in 2025 (cumulative **14,490**)
- **Iriscare mobility aids**: €5.86M for 40,234 services (2024)
- **STIB Taxibus**: end of PRM minibus end of 2027 (transition to private taxis, grant raised from €4.7M to €6.7M/year)
- **EAA** (European Accessibility Act) in force since 28 June 2025 — fines up to €200,000

See the cross-cutting dossier: [PRM accessibility](/en/dossiers/accessibilite-pmr).

## INAMI 2026 budget: €41.3bn

The INAMI General Council approved the 2026 healthcare budget at **€41.297bn** (+€1.566bn, growth norm 2% + indexation). Includes €150M of medical-services control, €50M for hospitals, €228M of pharmaceutical savings. Hospital cash flow: €580M of accelerated settlements in 2025, €275M expected in 2026.

Source: Le Spécialiste (2026); federal Public Health, e-Health 2026-2029 plan.

## ACS in the health-social sector

The **ACS** mechanism funds a significant share of staff in home help, early childcare and elderly care services. **Effort reduction €40M → €28M** (April 2026), **article 20 preserved**, **uniformisation at 95% abandoned**, **Actiris partnerships budget maintained at €67M**. The **Activa.brussels** premium is confirmed cancelled, replaced by a new scheme in 2027.

See the ACS dossier for the full analysis.

Source: Actiris / RPD; RTBF (April 2026).
