---
title: "Reform of the regional administration: from 25 entities to 4 pillars"
shortTitle: "Admin Reform"
seoTitle: "Brussels administration reform: from 25 entities to 4"
slug: reforme-administration
locale: en
dossierType: regulatory
phase: in-progress
crisisImpact: resolved
decisionLevel: regional
summary: "Restructuring of some twenty-five regional bodies into four pillars, with a target of 250 to 300 million euros of savings by 2029, including 67 million a year on consultancy. The Region employs around twelve thousand civil servants, up 37% since 2015, for a wage bill of 1.8 billion euros. The ratio stands at one civil servant per hundred inhabitants, against one per two hundred and eighty in Flanders and Wallonia. The hiring freeze remains in force."
summaryReviewed: "2026-08-30"
estimatedBudget: "250-300M EUR in targeted savings (2029)"
stakeholders:
  - "SPRB (Brussels Regional Public Service)"
  - "talent.brussels"
  - "perspective.brussels"
  - "urban.brussels"
  - "Bruxelles Environnement (IBGE)"
  - "Actiris"
  - "Innoviris"
  - "Parking.brussels"
  - "Court of Auditors"
  - "Trade unions (CSC, CGSP)"
relatedDomains:
  - budget
  - institutional
  - employment
  - digital
relatedSectors: []
relatedCommunes: []
relatedFormationEvents:
  - accord-gouvernement-bruxellois
sources:
  - label: "DH — From 25 entities to 4 pillars: the DPR on administrative simplification (13 Feb. 2026)"
    url: "https://www.dhnet.be/regions/bruxelles/2026/02/13/fusions-synergies-de-25-entites-a-4-piliers-voici-ce-que-contient-la-declaration-de-politique-regionale-en-matiere-de-simplification-administrative-C6C5M3DLXZGODNJLXAXAAJMRGU/"
    accessedAt: "2026-02-26"
  - label: "La Libre — The austerity cure will go through a Big Bang in the administration (13 Feb. 2026)"
    url: "https://www.lalibre.be/belgique/politique-belge/2026/02/13/la-cure-dausterite-de-la-region-bruxelloise-passera-par-un-big-bang-dans-son-administration-QTGDY3YNJRG6BFAEZQADJQO6ZY/"
    accessedAt: "2026-02-26"
  - label: "La Libre — No more soaring civil servant numbers in Brussels (19 Feb. 2026)"
    url: "https://www.lalibre.be/economie/conjoncture/2026/02/19/fini-la-hausse-parfois-vertigineuse-des-fonctionnaires-a-bruxelles-T4I4VVWBQRDYHKDTW5Z6LOTTQI/"
    accessedAt: "2026-02-26"
  - label: "RTBF — Betting on the administration for 500M in savings, a realistic target? (18 Feb. 2026)"
    url: "https://www.rtbf.be/article/region-bruxelloise-miser-sur-l-administration-pour-la-moitie-du-milliard-d-economies-un-objectif-realiste-11681725"
    accessedAt: "2026-02-26"
  - label: "DH — Public bodies under pressure facing the billion in savings (20 Feb. 2026)"
    url: "https://www.dhnet.be/regions/bruxelles/2026/02/20/les-organismes-publics-sous-pression-face-au-milliard-deconomies-du-gouvernement-bruxellois-il-va-falloir-se-montrer-inventif-creatif-et-convaincant-HJN74T5TCBGDNM7YGWGDHBDXLU/"
    accessedAt: "2026-02-26"
  - label: "talent.brussels — The Brussels civil service in figures (talentAnalytics 2024)"
    url: "https://www.talent.brussels/fr/actualites/la-fonction-publique-bruxelloise-en-chiffres-decouvrez-le-rapport"
    accessedAt: "2026-02-26"
  - label: "be.brussels — Administrations and institutions of the Region"
    url: "https://be.brussels/en/about-region/structure-and-organisations/administrations-and-institutions-region"
    accessedAt: "2026-02-26"
  - label: "Court of Auditors — Publications Brussels-Capital Region"
    url: "https://www.ccrek.be/FR/Publications/RegionDeBruxellesCapitale.html"
    accessedAt: "2026-02-26"
  - label: "BRUZZ — Regering Dilliès zet licht op groen voor begroting (5 maart 2026)"
    url: "https://www.bruzz.be/actua/politiek/regering-dillies-zet-licht-op-groen-voor-begroting-2026-03-05"
    type: media
    accessedAt: "2026-03-09"
  - label: "BRUZZ — Brusselse regering keurt eerste stap goed van hervorming administratie (5 June 2026)"
    url: "https://www.bruzz.be/actua/politiek/brusselse-regering-keurt-eerste-stap-goed-van-hervorming-administratie-2026-06-05"
    type: media
    accessedAt: "2026-06-07"
  - label: "La Libre — How the Brussels government plans to save one billion euros (13 Feb. 2026)"
    url: "https://www.lalibre.be/belgique/2026/02/13/vignette-auto-baremes-des-fonctionnaires-musee-kanal-voici-comment-le-gouvernement-bruxellois-compte-economiser-un-milliard-deuros-RZIQLKWMMNCMFDJDOVUIPATEZY/"
    accessedAt: "2026-06-14"
  - label: "BX1 — Over 100 million euros in consultancy fees in 5 years (14 Feb. 2024)"
    url: "https://bx1.be/categories/news/plus-de-100-millions-deuros-en-5-ans-depenses-en-frais-de-consultance-par-les-services-publics-bruxellois/"
    accessedAt: "2026-06-14"
  - label: "Le Vif — Over EUR 1 million for IT services: administrations still in the grip of consultancy (June 2026)"
    url: "https://www.levif.be/belgique/politique/regions/bruxelles/plus-d1-million-deuros-pour-des-services-informatiques-les-administrations-bruxelloises-toujours-sous-lemprise-de-la-consultance/"
    type: media
    accessedAt: "2026-06-14"
  - label: "BX1 — Merger looms and leadership vacant: unease grows at perspective.brussels (11 June 2026)"
    url: "https://bx1.be/categories/news/fusion-en-vue-et-direction-vacante-le-malaise-grandit-chez-perspective-brussels/"
    type: media
    accessedAt: "2026-07-07"
  - label: "BRUZZ — A 'competence pool' should help shift staff within Brussels administrations (3 July 2026)"
    url: "https://www.bruzz.be/actua/politiek/competentiepool-moet-helpen-om-met-personeel-te-schuiven-binnen-brusselse-administraties-2026-07-03"
    type: media
    accessedAt: "2026-07-05"
  - label: "BRUZZ: Headcount in Brussels public services falls, 'no more automatic replacements' (12 September 2026)"
    url: "https://www.bruzz.be/actua/politiek/personeelsbestand-brusselse-overheidsdiensten-daalt-geen-automatische-vervangingen-meer-2026-09-12"
    type: media
    accessedAt: "2026-09-13"
metrics:
  - label: "Current regional entities"
    value: "~25"
    source: "DH / DPR (Feb. 2026)"
    date: "2026-02-13"
  - label: "Target pillars"
    value: "4"
    source: "DH / DPR (Feb. 2026)"
    date: "2026-02-13"
  - label: "Regional civil servants"
    value: "~12,000"
    unit: "(+37% since 2015)"
    source: "talent.brussels (talentAnalytics 2024)"
    date: "2024-12-31"
  - label: "Wage bill"
    value: "1.8"
    unit: "bn EUR (+50% since 2018, incl. STIB-MIVB)"
    source: "La Libre (Feb. 2026)"
    date: "2024-12-31"
  - label: "Targeted savings"
    value: "250-300"
    unit: "M EUR by 2029"
    source: "DH / DPR (Feb. 2026)"
    date: "2026-02-13"
  - label: "Targeted consultancy savings"
    value: "67"
    unit: "M EUR/year by 2029"
    source: "La Libre (13 Feb. 2026)"
    url: "https://www.lalibre.be/belgique/2026/02/13/vignette-auto-baremes-des-fonctionnaires-musee-kanal-voici-comment-le-gouvernement-bruxellois-compte-economiser-un-milliard-deuros-RZIQLKWMMNCMFDJDOVUIPATEZY/"
    date: "2026-02-13"
  - label: "Private consultancy (2018-2022)"
    value: "≥107"
    unit: "M EUR (incl. ~50 to Deloitte)"
    source: "BX1 / openbudgets.brussels"
    url: "https://bx1.be/categories/news/plus-de-100-millions-deuros-en-5-ans-depenses-en-frais-de-consultance-par-les-services-publics-bruxellois/"
    date: "2024-02-14"
  - label: "Civil servants / residents ratio"
    value: "1 / 100"
    unit: "(vs 1/280 in Flanders/Wallonia)"
    source: "La Libre (Feb. 2026)"
    date: "2024-12-31"
  - label: "Hiring freeze"
    value: "Active"
    unit: "since Dec. 2023, extended indefinitely"
    source: "La Libre / DPR"
    date: "2023-12-01"
  - label: "Priority merger"
    value: "2026"
    unit: "perspective.brussels + urban.brussels"
    source: "DH / DPR (Feb. 2026)"
    date: "2026-02-13"
alerts:
  - label: "DPR announces administrative Big Bang: 25 entities to 4 pillars"
    severity: info
    date: "2026-02-13"
  - label: "Hiring freeze extended (active since Dec. 2023)"
    severity: warning
    date: "2023-12-01"
  - label: "Court of Auditors: adverse opinion on the 2024 accounts"
    severity: warning
    date: "2025-11-07"
  - label: "Merger of perspective + urban.brussels confirmed for 2026"
    severity: info
    date: "2026-02-13"
  - label: "Decree approved in first reading on 5 March 2026 (transversal administration)"
    severity: info
    date: "2026-03-05"
  - label: "perspective.brussels: Director-General mandate vacant since it expired due to the announced merger with urban.brussels, staff unease (11 June 2026)"
    severity: warning
    date: "2026-06-11"
  - label: "Launch of a regional 'pool de compétences' to redirect overstaffed personnel (social dialogue from 14 July 2026)"
    severity: info
    date: "2026-07-03"
confidenceLevel: estimated
dprCommitment: "Restructuring of the regional administration into 4 pillars, hiring freeze, shared support service, target of budgetary balance by 2029."
faq:
  - q: "What does the reform of the Brussels administration involve ?"
    a: "Announced in the Regional Policy Declaration, the reform aims to consolidate the current multitude of regional entities into four major pillars : a shared support service, the core missions of the regional public service, an infrastructure corporation and a land platform. The stated objective is to rationalise governance and generate savings. The first concrete operation is the merger of perspective.brussels and urban.brussels."
    sources:
      - label: "DH : from 25 entities to 4 pillars, the DPR on administrative simplification"
        url: "https://www.dhnet.be/regions/bruxelles/2026/02/13/fusions-synergies-de-25-entites-a-4-piliers-voici-ce-que-contient-la-declaration-de-politique-regionale-en-matiere-de-simplification-administrative-C6C5M3DLXZGODNJLXAXAAJMRGU/"
        accessedAt: "2026-06-17"
  - q: "Who decides on the reform, and at which level of power ?"
    a: "The reform falls under the regional level. The Brussels government sets the framework through decrees, subject to several readings, then transmits the parts that require a legal basis to the Brussels Parliament. Mergers of public-interest bodies require restructuring ordinances. The Court of Auditors checks the regional accounts and publishes its observations."
    sources:
      - label: "BRUZZ : the Brussels government approves the first step of the administration reform"
        url: "https://www.bruzz.be/actua/politiek/brusselse-regering-keurt-eerste-stap-goed-van-hervorming-administratie-2026-06-05"
        accessedAt: "2026-06-17"
      - label: "Court of Auditors : publications Brussels-Capital Region"
        url: "https://www.ccrek.be/FR/Publications/RegionDeBruxellesCapitale.html"
        accessedAt: "2026-06-17"
  - q: "Why does Brussels want to reduce its reliance on consultancy ?"
    a: "Cutting consultancy fees is presented as one of the reform's savings levers. Reliance on private audit, consulting and IT firms has long been documented by the press based on open budget data. The aim is to rebuild competences in-house and to limit spending considered significant, in a strained regional budgetary context."
    sources:
      - label: "BX1 : tens of millions in consultancy fees within Brussels public services"
        url: "https://bx1.be/categories/news/plus-de-100-millions-deuros-en-5-ans-depenses-en-frais-de-consultance-par-les-services-publics-bruxellois/"
        accessedAt: "2026-06-17"
      - label: "La Libre : how the Brussels government plans to make savings"
        url: "https://www.lalibre.be/belgique/2026/02/13/vignette-auto-baremes-des-fonctionnaires-musee-kanal-voici-comment-le-gouvernement-bruxellois-compte-economiser-un-milliard-deuros-RZIQLKWMMNCMFDJDOVUIPATEZY/"
        accessedAt: "2026-06-17"
  - q: "Does the reform lead to layoffs of civil servants ?"
    a: "No layoffs are announced. The main mechanism rests on a hiring freeze : departures (retirements, resignations) are no longer automatically replaced, with targeted recruitment still possible where the service requires it, internal mobility is preferred and the wage bill falls through natural attrition. Experts stress that this process is slow and uncertain, which weighs on the targeted savings trajectory."
    sources:
      - label: "La Libre : an end to the sharp rise in the number of civil servants in Brussels"
        url: "https://www.lalibre.be/economie/conjoncture/2026/02/19/fini-la-hausse-parfois-vertigineuse-des-fonctionnaires-a-bruxelles-T4I4VVWBQRDYHKDTW5Z6LOTTQI/"
        accessedAt: "2026-06-17"
      - label: "BRUZZ : headcount in Brussels public services falls, 'no more automatic replacements'"
        url: "https://www.bruzz.be/actua/politiek/personeelsbestand-brusselse-overheidsdiensten-daalt-geen-automatische-vervangingen-meer-2026-09-12"
        accessedAt: "2026-09-13"
lastModified: "2026-09-13"
faqReviewed: "2026-09-13"
changeType: updated
changeSummary: "The Region has more than 500 fewer full-time equivalents than in January 2026, according to BRUZZ. The Minister for the Public Service, quoted by the outlet, sees this as a first effect of the tightening of the hiring freeze in March, with 338 fewer full-time equivalents since then: departures are no longer automatically replaced, except where there is a proven need, as with the fire service."
changeSummaryDate: "2026-09-13"
---

## Hiring freeze: more than 500 full-time equivalents fewer since January 2026 (12 September 2026)

According to the Brussels Minister for the Budget and the Public Service, quoted by BRUZZ on Saturday 12 September 2026, the Region's workforce has fallen by more than 500 full-time equivalents since the start of the year, 338 of them in the four months since the hiring freeze was tightened in March 2026. Departures are no longer automatically replaced, but targeted recruitment remains possible where the service requires it, as at SIAMU, the fire service, which was given additional resources in the 2026 budget. The scope of this count is not specified: the Region employs around 12,000 staff. No official press release accompanied these figures.

Source: [BRUZZ (12 September 2026)](https://www.bruzz.be/actua/politiek/personeelsbestand-brusselse-overheidsdiensten-daalt-geen-automatische-vervangingen-meer-2026-09-12).

## The announced "Big Bang": from ~25 entities to 4 pillars

The Regional Policy Declaration (DPR) of 13 February 2026 announces a major restructuring of the Brussels administration. The objective: to consolidate approximately 25 regional entities into **4 major pillars**, aiming for 250 to 300 million EUR in savings by 2029.

### Pillar 1 — Shared support service

Pooling of cross-cutting functions: human resources (talent.brussels), IT (Paradigm, Connect IT), finance and budget, statistics (IBSA), equal opportunities (Equal.brussels), simplification (Easy.brussels).

### Pillar 2 — Core missions (SPRB)

Consolidation of policy directorates: Brussels International, Bruxelles Mobilite (administrative functions), Brussels Economy (with Innoviris, Screen.Brussels), Brussels Employment (integration of Actiris "under review"), Brussels Local Authorities, Brussels Housing (with Homegrade), **Brussels Urban Planning** (merger of perspective.brussels + urban.brussels — confirmed for 2026), Safe.Brussels.
Among these directorates, Brussels International absorbs the regional commissioner for Europe, whose post is to be abolished on 1 July 2026: see the [dedicated dossier](/en/dossiers/commissaire-europe).

### Pillar 3 — infrastructure.brussels

Public-law corporation bringing together operational services: Bruxelles Mobilite (construction, maintenance and DITP divisions), Port of Brussels (infrastructure), Bruxelles Environnement (operational services), Parking.brussels, LEZ management, ANPR cameras.

### Pillar 4 — Land platform

Coordination of land management: SAU (Urban Development Corporation), citydev.brussels, SLRB (social housing), Housing Fund, Port of Brussels (land concessions).

## Hiring freeze

The hiring freeze introduced in December 2023 by the outgoing government has been extended indefinitely:

- **2026**: general freeze (operational + non-operational); targeted recruitment remains possible where the service requires it, according to the Minister for the Public Service quoted by BRUZZ on 12 September 2026
- **From 2027**: freeze maintained except for operational functions
- **Mechanism**: no replacement of departures (retirements, resignations), internal mobility preferred
- **No layoffs** announced
- **Projected savings**: 50M EUR (2026), cumulating to 125M EUR (2029)
- Additional reduction of 25M EUR through the downsizing of ministerial cabinets

## A regional "pool de compétences" to absorb the hiring freeze (3 July 2026)

The Brussels Minister for the Public Service **Dirk De Smedt** (Anders) announced on **3 July 2026** the launch of a **"pool de compétences"** (Dutch: *competentiepool* — no official term has yet been announced in French, Dutch, English or German; BGM uses this working descriptive translation) intended to redirect staff from overstaffed administrations toward services facing a shortage of personnel.

The tool aims to structurally address the mismatch created by the **hiring freeze** (active since December 2023, see above): the freeze has reduced staff inflows but has not resolved the imbalance between administrations with surplus staff and those with genuine needs. Two tracks are planned:

- A **fast track**: volunteer staff go through a centralised matching procedure and can start, within 4 to 8 weeks, in a service with a concrete need, while keeping their status
- A **requalification track**, longer (6 to 18 months), for moves that require upskilling

**Social dialogue** on the project starts on **14 July 2026**, followed by a pilot phase to test the first tracks. Full operationalisation is targeted for **30 June 2027**. Trade unions are asking for prompt clarity on which administrations are affected.

Source: [BRUZZ (3 July 2026)](https://www.bruzz.be/actua/politiek/competentiepool-moet-helpen-om-met-personeel-te-schuiven-binnen-brusselse-administraties-2026-07-03). Confidence: unconfirmed (single source, no other outlet has picked up the information so far).

## First merger: perspective.brussels + urban.brussels (2026)

The merger of **perspective.brussels** (territorial development, 174 staff) and **urban.brussels** (urban planning and heritage) into a single entity called "Brussels Urban Planning" is the first concrete operation announced. It aims to create an **integrated permit** combining urban planning and environmental competences (currently split between Bruxelles Environnement and urban.brussels).

The case of **Actiris** (1,518 staff, headcount doubled in 20 years) has been identified as a potential integration into "Brussels Employment", but remains at the study stage.

## Decree approved in first reading (5 March 2026)

The Dilliès government approved on **5 March 2026**, in first reading, a decree creating a large transversal administration. This project concretises Pillar 1 (common support service) by consolidating transversal functions (HR, IT, finance, budget, statistics) under a single structure, placed under the authority of Minister of Public Service Dirk De Smedt (Anders).

This is the **first formal legislative act** of the 4-pillar reform. The second reading is expected in the following weeks.

Sources: Le Soir, BRUZZ (5 March 2026).

**Second reading — 5 June 2026**: The Brussels government approved the second reading of the first pillar (shared support service). More than a hundred civil servants from different administrations took part in the preparation; the government intends to permanently anchor the new structure by the summer of 2026. Next step: submission to the Brussels Parliament.

## Consultancy in the crosshairs

Cutting consultancy fees is one of the savings levers the government is putting forward: **up to EUR 67 million a year** would be saved on this item by 2029, within a EUR 281 million package of "operational resource optimisation".

The issue is long-documented. According to openbudgets.brussels data relayed by the press, Brussels public services spent **at least EUR 107 million on private audit and consulting firms over 2018-2022**, of which roughly **EUR 50 million for Deloitte alone** (ahead of KPMG, EY and PwC). A study by the PTB put the 2019-2024 total at around EUR 470 million.

In June 2026, a press investigation (Le Vif) found that reliance on IT consultancy was continuing — over EUR 300,000 earmarked for 1.5 full-time equivalents over a year in a regional contract — illustrating the difficulty of meeting the announced trajectory. *(Press data, single-source, to be corroborated via public tenders.)*

## Workforce trajectory: 243 staff (1989) to 12,000 (2024)

The Brussels regional administration has experienced continuous growth since the creation of the Region in 1989:

- **1989**: 243 staff at the Ministry of the Brussels-Capital Region (MRBC)
- **2015**: 8,756 regional civil servants
- **2023**: 12,093 civil servants (+37% in 8 years)
- **Wage bill** (incl. STIB-MIVB): from 1.2 bn EUR (2018) to 1.8 bn EUR (2024), i.e. +50%
- **Ratio**: 1 regional civil servant per 100 residents (vs 1/280 in Flanders and Wallonia)

This growth is explained by successive transfers of competences (6th Reform of the State, 2014), the dissolution of the Province of Brabant (1993), and the continuous creation of new bodies: visit.brussels, Innoviris, parking.brussels, perspective.brussels, hub.brussels, etc.

## Financial context: adverse opinion from the Court of Auditors

The 30th Book of Observations by the Court of Auditors (November 2025) issued an **adverse opinion** on the Region's 2024 accounts, identifying "significant anomalies with a diffuse impact" across the 21 autonomous administrative bodies. Key figures:

- **Actual deficit**: 1.65 bn EUR (31.5% of revenues)
- **Consolidated gross debt**: 15.6 bn EUR (+76.7% over 2020-2024)
- **Interest charges**: from 91M EUR (2016) to 399M EUR (2024), i.e. x4.4

## Documented scepticism

Several experts and institutional actors have expressed reservations about the feasibility of the reform:

- **Prof. Marie Goransson (ULB)**: "Even with all of this, you still don't reach 500 million in savings." She estimates that without layoffs, savings depend on natural attrition — a slow and uncertain process.
- **CSC (trade union)**: questions about the physical consolidation of buildings and transitional protections for staff.
- **Voka**: "The agreement contains the right ingredients, but questions remain about the budgetary trajectory."

## Announced timeline

- **2026**: launch of pillars 1 and 2, priority to the perspective + urban.brussels merger
- **2027**: start of pillar 3 reforms (infrastructure.brussels)
- **2029**: target for regional budgetary balance

## perspective.brussels leadership: mandate vacant since the announced merger (June-July 2026)

The Director-General mandate at **perspective.brussels** (Antoine de Borman) has expired without the Brussels government deciding on its renewal, precisely because of the announced merger with urban.brussels (see Pillar 2 above). On **11 June 2026**, a staff gathering voiced concern over this leadership vacancy and the broader uncertainty surrounding the reform. The Secretary of State in charge of the dossier, **Audrey Henry**, acknowledged the concerns and promised consultations, without announcing a decision.

**Why flag this**: the mechanism is the same one that led to the abolition of the regional commissioner to Europe post (see [dedicated dossier](/en/dossiers/commissaire-europe)) — institutional simplification makes leadership mandates redundant once two entities merge. But unlike the commissioner-to-Europe case, **no abolition decision has been officially announced at this stage**: this is a frozen/vacant mandate pending the merger, not a scrapped position. BGM classifies this as "developing," not to be confused with a closed case.

Source: [BX1 (11 June 2026)](https://bx1.be/categories/news/fusion-en-vue-et-direction-vacante-le-malaise-grandit-chez-perspective-brussels/). Confidence: official (vacancy noted, staff gathering); unconfirmed (outcome of the merger, final decision on the mandate).

## Issues to watch

- **Restructuring ordinances**: legal framework required for mergers of autonomous public bodies (OIP)
- **Merger of perspective + urban.brussels**: first concrete test (2026)
- **Actiris headcount**: integration into "Brussels Employment" — decision pending
- **talent.brussels report**: annual staffing data (attrition monitoring)
- **Court of Auditors**: next book of observations (monitoring of achieved savings)
- **Trade union reactions**: negotiations on transitional protections
