  ...
  return (
    <form onSubmit={handleSave} className="rounded-lg border border-neutral-200 bg-white p-6">
      ...
      <fieldset className="mb-6">
        ...
        <p className="mb-1.5 text-xs text-neutral-500">{t('domainsGroup')}</p>
        ...
        <p className="mt-3 mb-1.5 text-xs text-neutral-500">{t('sectorsGroup')}</p>
        <div className="flex flex-wrap gap-2">
          {SECTOR_TOPICS.map((topic) => (
            <label
              key={topic}
              className={`cursor-pointer rounded-full border px-3 py-1 text-xs font-medium transition-colors ${
                topics.includes(topic)
        ...
        <p className="mt-3 mb-1.5 text-xs text-neutral-500">{t('engagementsGroup')}</p>
        <div className="flex flex-wrap gap-2">
          {ENGAGEMENT_TOPICS.map((topic) => (
            <label
              key={topic}
              className={`cursor-pointer rounded-full border px-3 py-1 text-xs font-medium transition-colors ${
                topics.includes(topic)
