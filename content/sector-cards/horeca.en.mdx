---
title: "Horeca: hotel VAT 12%, record bankruptcies, Visit.brussels austerity, Pentagon begging regulation"
slug: horeca
locale: en
parentDomain: employment
sector: "Hotels, restaurants and cafés"
frozenMechanisms: []
activeMechanisms:
  - name: "Existing employment aid"
    description: "Employment support measures committed before 2026 continue to be implemented by Actiris."
  - name: "Tourism promotion"
    description: "Visit.brussels continues its tourism promotion mission within the reduced 2026 budget (€22M → €16.3M)."
impactIndicators:
  - label: "Horeca establishments in Brussels"
    value: "~9,000"
    source: "hub.brussels"
    url: "https://hub.brussels/"
    frequency: "annuel"
  - label: "Direct jobs in Brussels horeca"
    value: "~35,000"
    source: "Actiris / ONSS"
    url: "https://www.actiris.brussels/"
    frequency: "annuel"
  - label: "Tourist nights in Brussels (2024)"
    value: "~8.5 million"
    source: "Visit.brussels"
    url: "https://visit.brussels/"
    frequency: "annuel"
  - label: "Total Brussels bankruptcies (2025)"
    value: "2,184"
    source: "BX1"
    frequency: "annuel"
  - label: "Share of horeca bankruptcies (Brussels)"
    value: "17% (vs 6% of SMEs)"
    source: "hub.brussels / BX1"
    frequency: "annuel"
  - label: "Horeca jobs lost (2025, Belgium)"
    value: "5,631"
    source: "BX1"
    frequency: "annuel"
  - label: "Hotel VAT (1 March 2025)"
    value: "6 → 12%"
    source: "Federal government / programme law"
    frequency: "ponctuel"
  - label: "Visit.brussels — Place Royale desk closed (April 2026)"
    value: "1 of 2 tourist desks"
    source: "BRUZZ Economy (20 April 2026)"
    url: "https://www.bruzz.be/actua/samenleving/besparingen-dwingen-visit-brussels-tot-sluiting-toerismebureau-koningsplein-2026-04-20"
    frequency: "ponctuel"
  - label: "Visit.brussels — 2026 budget (austerity)"
    value: "€22M → €16.3M"
    source: "BGM culture dossier (March 2026)"
    frequency: "annuel"
  - label: "Visit.brussels — last tourist desk permanently closed (30 June 2026)"
    value: "0 physical tourist desks left in Brussels"
    source: "BRUZZ"
    url: "https://www.bruzz.be/actua/economie/visitbrussels-sluit-toeristisch-kantoor-op-de-grote-markt-door-besparingen-2026-06-30"
    frequency: "ponctuel"
  - label: "Flexi-jobs — federal extension to all sectors (1 July 2026)"
    value: "Chamber: favourable vote on 19 June 2026"
    source: "RTBF"
    url: "https://www.rtbf.be/article/emploi-la-chambre-donne-son-glue-a-l-extension-des-flexi-jobs-a-tous-les-secteurs-entree-en-vigueur-prevue-le-1er-juillet-11743573"
    frequency: "ponctuel"
stakeholders:
  - name: "Brussels Horeca Federation"
    type: federation
    url: "https://www.horecabruxelles.be/"
  - name: "Actiris"
    type: agency
    url: "https://www.actiris.brussels/"
  - name: "Visit.brussels"
    type: agency
    url: "https://visit.brussels/"
  - name: "hub.brussels"
    type: agency
    url: "https://hub.brussels/"
humanImpact: "Hotel VAT raised from 6% to 12% since 1 March 2025. 17% of Brussels bankruptcies are in horeca (vs 6% of SMEs). Visit.brussels closed its last physical tourist desk (Grand-Place, 30 June 2026): Brussels no longer has a single tourist welcome point. The federal extension of flexi-jobs to all sectors (1 July 2026) dilutes horeca's historical advantage. Brussels Horeca Federation denounces a «broken contract» with the political world (July 2026). The Brussels-City Pentagon begging regulation (20 April 2026) is now the subject of a legal challenge before the Council of State."
lastModified: "2026-07-20"
changeType: updated
changeSummary: "Correction: the Grand-Place tourist desk, previously described as retained, in fact permanently closed on 30 June 2026, Brussels no longer has any physical tourist welcome point. The federal government extended flexi-jobs to all sectors (1 July 2026), diluting horeca's historical advantage. The Brussels Horeca Federation sent an open letter (15 July 2026) describing a «broken contract» with the political world. Seven associations filed a challenge before the Council of State against the Pentagon begging regulation."
summaryFalc: "Hotels have paid 12% VAT instead of 6% since 1 March 2025. Horeca accounts for 17% of Brussels bankruptcies. The last Visit.brussels tourist desk permanently closed in June 2026. Flexi-jobs are now open to all sectors, not just horeca. The Brussels Horeca Federation is angry with the political world."
---

## Hotel VAT: 6 → 12% since 1 March 2025

The federal programme law raised the **VAT on hotel accommodation** from 6% to 12% on **1 March 2025**. The Horeca Federation flagged the competitive impact versus European destinations (Paris, Amsterdam, Berlin), notably for **MICE tourism** (meetings, conferences, business events). 2024 tourist nights were around 8.5 million in Brussels.

Sources: 2025 federal programme law; Brussels Horeca Federation.

## Regional tourism austerity: Visit.brussels shrinks

The Brussels government cut the **Visit.brussels budget from €22M to €16.3M** (−26%) for 2026. Immediate consequences:

- **20 April 2026**: closure of the **Place Royale tourist desk** (Brussels Info Place / Koningsplein). Only the Grand-Place desk remains.
- **BD Comic Strip Festival 2026 cancelled** (March 2026)
- **Iris Festival** scaled back

For horeca, the issue is the **welcoming of foreign tourists** at a time when VAT rises — a contradictory signal sent to an already fragile sector.

Source: [BRUZZ Economy](https://www.bruzz.be/actua/samenleving/besparingen-dwingen-visit-brussels-tot-sluiting-toerismebureau-koningsplein-2026-04-20) (20 April 2026).

## Bankruptcies: 17% horeca for 6% of SMEs

Brussels recorded around **2,184 bankruptcies** in 2025 (all activities combined). Horeca, accounting for only **6% of SMEs**, concentrates **17% of bankruptcies** — nearly 3× its share of the economy. At the Belgian level, the sector lost **5,631 jobs** in 2025.

Stacking factors: COVID, inflation, staff shortages, rising commercial rents, hotel VAT, regional budget austerity.

Source: BX1; Brussels Horeca Federation.

## Pentagon begging regulation (Brussels-City, 20 April 2026)

The City of Brussels Municipal Council adopted on **20 April 2026** a regulation banning begging with children under 16 in the **Pentagon**, Place Louise and Avenue Louise (fines up to €500), and "aggressive, insistent, intimidating" begging across the City for 2 years. Direct impact on the Pentagon's commercial atmosphere — a horeca concentration zone.

Source: [La Libre](https://www.lalibre.be/regions/bruxelles/2026/04/20/bruxelles-serre-la-vis-sur-la-mendicite-avec-enfants-lopposition-denonce-un-texte-cruel-Z3JMT7WPRRDJFPPRAIHMEALUDI/) (20 April 2026).

## Coalition agreement measures affecting horeca

- **Annual LEZ pass €350** (social €200), per-infraction fine reduced from €350 to €80 — predictable cost for suppliers and delivery
- **Plan succeeding Good Move** — the revision of mobility layouts will affect horeca zones in redesigned grids
- **Administrative reform (Big Bang 25 → 4 pillars)** — Hub.brussels and Visit.brussels are part of the consolidation, schedule to be detailed

## Bpost: extended strike, impact on horeca

The Bpost strike (mid-April 2026, 18-19 days) blocked deliveries in Brussels. For horeca, consequences hit **supplier invoices**, **paper orders** and **communication with regular customers**. CEO Peeters proposed exceptional 5-6 PM hours ("childcare buffer").

Source: BGM employment dossier / V2 memory (April 2026).
