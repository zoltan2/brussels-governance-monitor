---
title: "Le vice-gouverneur de Bruxelles : le gardien méconnu des lois linguistiques"
shortTitle: "Vice-gouverneur"
seoTitle: "Vice-gouverneur de Bruxelles, gardien des lois linguistiques"
slug: vice-gouverneur
locale: fr
dossierType: regulatory
phase: in-progress
crisisImpact: unaffected
decisionLevel: mixed
summary: "Bruxelles a un « vice-gouverneur » mais plus de gouverneur. Nommé par la Région sur avis conforme du fédéral, il n'a qu'une mission : contrôler le respect des lois linguistiques dans les 19 communes et leurs CPAS. Il peut suspendre une nomination non conforme, mais pas l'annuler — si bien qu'en 2024 à peine 15,4 % des dossiers contrôlés étaient conformes et que les communes reconfirment presque toujours. Un poste discret, à 4 agents, dont le rôle réel interroge."
summaryReviewed: "2026-06-15"
faqReviewed: "2026-09-22"
estimatedBudget:
  status: unpublished
  reason: "le service ne communique aucun budget de fonctionnement ; effectif : 4 agents"
stakeholders:
  - "Vice-gouverneur de l'arrondissement administratif de Bruxelles-Capitale"
  - "19 communes bruxelloises"
  - "19 CPAS bruxellois"
  - "Gouvernement de la Région de Bruxelles-Capitale (tutelle / annulation)"
  - "Commission permanente de contrôle linguistique (CPCL)"
  - "Parlement bruxellois (destinataire du rapport annuel)"
  - "État fédéral (avis conforme à la nomination)"
relatedDomains:
  - institutional
  - employment
relatedSectors: []
relatedCommunes: []
relatedFormationEvents: []
sources:
  - label: "Vice-gouverneur de Bruxelles-Capitale — Rapports annuels (PDF officiels)"
    url: "https://vicegouverneurbru.be/jaarverslagen/2025FR.pdf"
    accessedAt: "2026-06-14"
  - label: "Vice-gouverneur — Rapport annuel 2024 (PDF officiel)"
    url: "https://vicegouverneurbru.be/jaarverslagen/2024FR.pdf"
    accessedAt: "2026-06-14"
  - label: "CRISP — Vocabulaire politique : « vice-gouverneur »"
    url: "https://www.vocabulairepolitique.be/vice-gouverneur/"
    accessedAt: "2026-06-14"
  - label: "Lois coordonnées du 18 juillet 1966 sur l'emploi des langues en matière administrative (art. 65) — ejustice"
    url: "https://www.ejustice.just.fgov.be/cgi_loi/change_lg.pl?language=fr&la=F&cn=1966071831&table_name=loi"
    accessedAt: "2026-06-14"
  - label: "Loi spéciale du 12 janvier 1989 relative aux institutions bruxelloises — Parlement bruxellois (PDF)"
    url: "http://www.parlement.brussels/wp-content/uploads/2017/06/l89.pdf"
    accessedAt: "2026-06-14"
  - label: "RTBF — Emploi des langues : des centaines de nominations illégales (18 mars 2016, maj 25 fév. 2025)"
    url: "https://www.rtbf.be/article/emploi-des-langues-des-centaines-de-nominations-illegales-dans-les-administrations-bruxelloises-9243158"
    type: media
    accessedAt: "2026-06-14"
  - label: "BX1 — Près de 2 000 recrutements suspendus à Bruxelles pour non-respect de la législation linguistique (2021)"
    url: "https://bx1.be/categories/news/pres-de-2-000-recrutements-suspendus-a-bruxelles-pour-non-respect-de-la-legislation-linguistique/"
    type: media
    accessedAt: "2026-06-14"
  - label: "DaarDaar (trad. BRUZZ) — Bruxelles, une région bilingue ? Sur le papier uniquement (11 juillet 2022)"
    url: "https://daardaar.be/rubriques/culture-et-medias/bruxelles-une-region-bilingue-sur-le-papier-uniquement-et-la-situation-empire/"
    type: media
    accessedAt: "2026-06-14"
  - label: "Adjoint du gouverneur du Brabant flamand — Compétences (équivalent en Région flamande)"
    url: "https://www.adjunctvandegouverneur.be/fr/information/notre-service/competences"
    accessedAt: "2026-06-14"
  - label: "Bruxelles Pouvoirs Locaux — La tutelle administrative"
    url: "https://pouvoirs-locaux.brussels/la-tutelle-administrative"
    accessedAt: "2026-06-14"
  - label: "Commission permanente de contrôle linguistique (CPCL)"
    url: "https://www.vct-cpcl.be/fr/commissie"
    accessedAt: "2026-06-14"
metrics:
  - label: "Dossiers de recrutement contrôlés"
    value: "3 199"
    unit: "en 2025 (communes + CPAS)"
    source: "Vice-gouverneur — Rapport annuel 2025"
    url: "https://vicegouverneurbru.be/jaarverslagen/2025FR.pdf"
    date: "2026-05-13"
  - label: "Dossiers conformes aux lois linguistiques"
    value: "15,4 %"
    unit: "en 2024 (560 sur 3 639)"
    source: "Vice-gouverneur — Rapport annuel 2024"
    url: "https://vicegouverneurbru.be/jaarverslagen/2024FR.pdf"
    date: "2025-03-31"
  - label: "Suspensions de recrutement"
    value: "1 867"
    unit: "en 2020 (record sur la décennie)"
    source: "Vice-gouverneur — Rapport annuel 2020 / BX1"
    url: "https://bx1.be/categories/news/pres-de-2-000-recrutements-suspendus-a-bruxelles-pour-non-respect-de-la-legislation-linguistique/"
    date: "2021-06-26"
  - label: "Suspensions suivies d'une annulation"
    value: "≈ 2"
    unit: "(les communes reconfirment presque toujours)"
    source: "RTBF (2016, maj 2025)"
    url: "https://www.rtbf.be/article/emploi-des-langues-des-centaines-de-nominations-illegales-dans-les-administrations-bruxelloises-9243158"
    date: "2025-02-25"
  - label: "Plaintes linguistiques traitées"
    value: "61"
    unit: "en 2024 (record ; 5 en 2016)"
    source: "Vice-gouverneur — Rapport annuel 2024"
    url: "https://vicegouverneurbru.be/jaarverslagen/2024FR.pdf"
    date: "2025-03-31"
  - label: "Effectif du service"
    value: "4"
    unit: "agents (qualifié de « critique » chaque année)"
    source: "Vice-gouverneur — Rapports annuels 2016-2025"
    url: "https://vicegouverneurbru.be/jaarverslagen/2025FR.pdf"
    date: "2026-05-13"
  - label: "Délai de suspension"
    value: "40"
    unit: "jours après réception de l'acte"
    source: "Lois coordonnées 18 juillet 1966, art. 65"
    url: "https://www.ejustice.just.fgov.be/cgi_loi/change_lg.pl?language=fr&la=F&cn=1966071831&table_name=loi"
    date: "1966-07-18"
  - label: "Création de la fonction"
    value: "1963"
    unit: "(recentrée sur Bruxelles à la scission du Brabant, 1995)"
    source: "CRISP — Vocabulaire politique"
    url: "https://www.vocabulairepolitique.be/vice-gouverneur/"
    date: "1995-01-01"
alerts:
  - label: "Conformité linguistique très faible : 15,4 % des dossiers conformes (2024)"
    severity: warning
    date: "2025-03-31"
  - label: "Pouvoir de suspension sans annulation : effet pratique limité"
    severity: info
    date: "2026-06-14"
  - label: "Chevauchement de tutelle reconnu par Bruxelles Pouvoirs Locaux"
    severity: info
    date: "2026-06-14"
confidenceLevel: official
faq:
  - q: "Qu'est-ce que le vice-gouverneur de Bruxelles ?"
    a: "Le vice-gouverneur est le commissaire chargé de contrôler le respect des lois sur l'emploi des langues dans les 19 communes bruxelloises et leurs CPAS. Bruxelles dispose d'un vice-gouverneur mais d'aucun gouverneur : depuis la sixième réforme de l'État, les missions de sécurité et de gestion de crise sont confiées à un haut fonctionnaire. La fonction de vice-gouverneur, elle, a été maintenue, avec cette mission linguistique unique."
    sources:
      - label: "CRISP, Vocabulaire politique : vice-gouverneur"
        url: "https://www.vocabulairepolitique.be/vice-gouverneur/"
        accessedAt: "2026-06-17"
  - q: "À quoi sert le vice-gouverneur de Bruxelles ?"
    a: "Il vérifie la conformité linguistique des actes des communes et des CPAS, en particulier les nominations et recrutements : la personne engagée détient-elle le brevet de connaissance de la seconde langue exigé à Bruxelles ? Son rôle touche un principe fondateur de la capitale : le bilinguisme du service public, c'est-à-dire le droit du Bruxellois d'être accueilli et servi en français ou en néerlandais par sa commune et son CPAS."
    sources:
      - label: "Lois coordonnées du 18 juillet 1966 sur l'emploi des langues en matière administrative, art. 65"
        url: "https://www.ejustice.just.fgov.be/cgi_loi/change_lg.pl?language=fr&la=F&cn=1966071831&table_name=loi"
        accessedAt: "2026-06-17"
  - q: "Qui nomme le vice-gouverneur de Bruxelles ?"
    a: "Le vice-gouverneur est nommé et révoqué par le gouvernement de la Région de Bruxelles-Capitale, sur avis conforme du gouvernement fédéral. C'est donc une fonction d'origine fédérale exercée à l'échelon régional. Le titulaire doit justifier d'une connaissance approfondie du français et du néerlandais."
    sources:
      - label: "Loi spéciale du 12 janvier 1989 relative aux institutions bruxelloises, Parlement bruxellois"
        url: "http://www.parlement.brussels/wp-content/uploads/2017/06/l89.pdf"
        accessedAt: "2026-06-17"
  - q: "Le vice-gouverneur peut-il annuler une nomination communale ?"
    a: "Non. Son seul outil est la suspension : dans les quarante jours suivant la réception d'un acte, il peut, par arrêté motivé, en suspendre l'exécution. L'annulation, elle, relève du gouvernement régional, autorité de tutelle. Le vice-gouverneur n'est donc qu'un maillon de la chaîne : il signale, suspend et transmet, mais c'est un autre niveau de pouvoir qui tranche."
    sources:
      - label: "Lois coordonnées du 18 juillet 1966 sur l'emploi des langues en matière administrative, art. 65"
        url: "https://www.ejustice.just.fgov.be/cgi_loi/change_lg.pl?language=fr&la=F&cn=1966071831&table_name=loi"
        accessedAt: "2026-06-17"
lastModified: "2026-08-09"
changeType: added
changeSummary: "Le dossier de la gare de Bockstael, à Laeken, donne à ce dossier son cas d'école. Enquête publique menée uniquement en néerlandais, plainte d'un riverain déclarée recevable et fondée par la Commission permanente de contrôle linguistique, report refusé en commission de concertation, permis délivré en néerlandais seul, deuxième plainte, recours au Conseil d'État. Et en août 2026, le chantier démarre quand même, parce que le recours n'est pas suspensif. C'est la démonstration la plus nette de ce que ce dossier explique : un avis fondé de la commission n'annule rien, ne suspend rien et n'oblige personne."
---

## Cas d'école : la gare de Bockstael, ou ce que vaut un avis fondé (août 2026)

Le dossier de la rénovation de la **gare de Bockstael**, à Laeken, illustre en pratique ce que ce dossier décrit en théorie : la chaîne du contrôle linguistique produit des constats, pas des effets.

L'enquête publique s'est tenue avec un dossier rédigé **uniquement en néerlandais**. Un riverain a saisi la **CPCL**, qui a déclaré sa plainte **recevable et fondée** : il aurait dû recevoir les documents en français. La doctrine de la commission est constante depuis 1994 et rappelée dans son avis n° 56.353 du 21 octobre 2024 : le demandeur dépose dans une seule langue, mais **l'autorité qui organise l'enquête publique** doit mettre les documents essentiels à disposition dans les deux langues.

Ce qui s'est passé ensuite :

- la demande de report en commission de concertation a été **rejetée** ;
- le permis a été délivré **en néerlandais seul** ;
- une deuxième plainte a été déposée devant la CPCL et un **recours introduit au Conseil d'État** ;
- en **août 2026**, le permis modificatif est toujours unilingue et **le chantier a démarré**, Infrabel invoquant le caractère non suspensif du recours.

C'est la démonstration la plus nette du point central de ce dossier : un avis fondé de la CPCL **n'annule rien, ne suspend rien, n'oblige personne**. Seul le Conseil d'État peut annuler, et il statue après les travaux.

[Voir la carte Institutions →](/fr/domaines/institutional)

Sources : [CPCL, avis n° 56.353 du 21 octobre 2024 (PDF)](https://vct-cpcl.be/sites/default/files/documenten/56353-x-nb-az_0.pdf) ; [La DH (7 août 2026)](https://www.dhnet.be/regions/bruxelles/bruxelles-mobilite/2026/08/07/polemique-linguistique-autour-de-la-gare-bockstael-a-laeken-il-y-a-une-flamandisation-du-dossier-qui-saccompagne-dune-passivite-francophone-ZNKWK4TTQ5HGXFXBNEBOL2MMXQ/) ; [La Libre (5 février 2024)](https://www.lalibre.be/regions/bruxelles/2024/02/05/rififi-autour-de-la-renovation-de-la-gare-de-bockstael-nous-sommes-a-bruxelles-jai-le-droit-davoir-les-documents-en-francais-LUJM3ZEVFBFI7OH4SEO2LXPHYY/). Confiance : officielle pour l'avis n° 56.353 et sa jurisprudence ; les décisions propres au dossier Bockstael sont rapportées par la presse, les avis individuels de la CPCL n'étant pas systématiquement publiés.

## Bruxelles a un vice-gouverneur, mais plus de gouverneur

C'est l'une des curiosités institutionnelles de la capitale : Bruxelles dispose d'un **vice-gouverneur**, mais d'aucun gouverneur. La fonction de gouverneur a été supprimée par la sixième réforme de l'État (loi spéciale du 6 janvier 2014) ; ses missions de sécurité, de planification d'urgence et de gestion de crise sont depuis exercées par un **haut fonctionnaire**. Le vice-gouverneur, lui, subsiste — avec une mission unique et très ciblée : **veiller au respect des lois sur l'emploi des langues** dans les 19 communes et les 19 CPAS de l'arrondissement administratif de Bruxelles-Capitale.

Le grand public ignore largement l'existence de ce poste. Ce dossier propose de l'objectiver : d'où vient-il, que fait-il vraiment, avec quels moyens, et en quoi aide-t-il (ou non) le Bruxellois ?

## Des origines communautaires : 1963, puis 1995

La fonction naît de la **loi du 2 août 1963** sur l'emploi des langues en matière administrative, en pleine pacification communautaire (fixation de la frontière linguistique, délimitation des 19 communes bruxelloises bilingues). À l'époque, Bruxelles fait partie de la **province de Brabant** : on crée un commissaire du gouvernement portant le titre de *vice-gouverneur du Brabant*, spécifiquement chargé du contrôle linguistique, distinct du gouverneur de province.

Tout bascule avec la **scission de la province de Brabant au 1ᵉʳ janvier 1995** (quatrième réforme de l'État). Le Brabant unitaire éclate en trois : Brabant wallon, Brabant flamand et **arrondissement administratif de Bruxelles-Capitale**, qui n'appartient désormais à aucune province. Les compétences de l'ancien vice-gouverneur sont réparties entre le **vice-gouverneur de Bruxelles** et l'**adjoint du gouverneur du Brabant flamand** (compétent pour les six communes à facilités de la périphérie).

## Les vice-gouverneurs successifs

Depuis la création de l'arrondissement en 1995, trois personnes ont occupé la fonction :

- **Hugo Nys** (1995 – 2010) — nommé ; également gouverneur faisant fonction à partir de 2009, jusqu'à son départ à la retraite.
- **Jean Clément** (2010 – 2012) — faisant fonction (il cumule alors les fonctions de gouverneur et de vice-gouverneur faisant fonction).
- **Jozef Ostyn** (depuis 2012) — faisant fonction dès février 2012, puis nommé définitivement en 2013 ; toujours en poste.

## Ce qu'il fait — et ce qu'il ne peut pas faire

Le vice-gouverneur contrôle la conformité linguistique des **actes des communes et des CPAS**, en particulier les **nominations et recrutements** d'agents : la personne engagée détient-elle le brevet de connaissance de la seconde langue exigé pour un poste à Bruxelles ?

Son outil : la **suspension**. Dans les **40 jours** suivant la réception d'un acte (article 65 des lois coordonnées du 18 juillet 1966), il peut, par arrêté motivé, en suspendre l'exécution. Mais — et c'est le cœur du problème — **il ne peut pas annuler**. L'annulation relève du **gouvernement régional** (autorité de tutelle). Le vice-gouverneur n'est donc qu'un maillon d'une chaîne : il signale, suspend, transmet ; un autre tranche.

Il établit chaque année un **rapport** transmis au Parlement bruxellois, et il **supplée** le haut fonctionnaire/gouverneur en cas d'absence. Il est **nommé et révoqué par le gouvernement régional sur avis conforme du gouvernement fédéral**, et doit justifier d'une connaissance approfondie du français et du néerlandais.

## Les chiffres : un contrôle massif, une conformité minime

Les rapports annuels (2016-2025) dressent un tableau constant et frappant.

- **Volume contrôlé en hausse puis reflux** : de **2 329 dossiers en 2016** à un pic de **3 758 en 2023**, puis **3 199 en 2025**. Les CPAS ont longtemps tiré le volume (pic de 2 020 dossiers en 2020), avant un reflux côté communes en 2025.
- **Une conformité très faible** : en **2024, seuls 560 dossiers sur 3 639 étaient conformes**, soit **15,4 %**. L'essentiel des suspensions concerne le **personnel contractuel des CPAS**, où le brevet linguistique fait massivement défaut.
- **Des suspensions records** : **1 867 suspensions de recrutement en 2020**, niveau jamais atteint.
- **Des plaintes en forte hausse** : de **5 plaintes en 2016** à un **record de 61 en 2024** (le rapport n'y trouve « pas d'explication univoque »), retombées à 32 en 2025.

Autrement dit : le service contrôle énormément, constate une non-conformité écrasante… mais sans pouvoir y mettre fin.

## L'influence réelle : une « fonction d'alerte »

Le pouvoir de suspension est, en pratique, largement symbolique. Selon la presse de référence, **à peine deux suspensions ont été suivies d'une annulation effective** ; l'agent recruté reste généralement en poste « pour assurer la continuité du service ». Le titulaire le reconnaît lui-même : si une nomination est suspendue, les administrations locales **reconfirment presque systématiquement** leur décision, et au niveau régional « on attend l'échéance du délai ». Il décrit son rôle comme une **« fonction d'alerte »** plutôt que comme un pouvoir contraignant.

Les **moyens** confortent ce constat : le service fonctionne avec **4 agents**, un effectif que les rapports qualifient de **« critique »** année après année, avec un poste durablement vacant et des demandes répétées de renfort. **Aucun budget de fonctionnement n'est publié** dans les rapports — seule apparaît, à partir de 2023, la mention d'un besoin d'« apport financier substantiel », non chiffré.

## Un poste unique… mais pas tout à fait

Le vice-gouverneur est-il une singularité bruxelloise ?

- **En Belgique** : son seul véritable jumeau est l'**adjoint du gouverneur du Brabant flamand**, créé en 1995, qui dispose du même pouvoir de suspension pour les six communes à facilités de la périphérie. La différence est le contexte : l'adjoint protège une minorité francophone en territoire unilingue néerlandophone ; le vice-gouverneur contrôle le bilinguisme dans une région officiellement bilingue. Ailleurs (Fourons, Comines, cantons de l'Est), le contrôle linguistique passe par de simples commissaires d'arrondissement, sans pouvoir de suspension homonyme.
- **En Europe** : aucun équivalent exact. Les territoires multilingues confient généralement la garantie linguistique soit à un **ombudsman** (Canada, Irlande, Nouveau-Brunswick), soit à un mécanisme **structurel** de quotas (Tyrol du Sud), soit au **juge**. Le couple « protection d'une langue localement minoritaire + pouvoir de suspendre des actes administratifs publics » est rarissime.

## La « lasagne » : un chevauchement assumé

Le contrôle linguistique du vice-gouverneur est une **tutelle distincte et parallèle** à la tutelle administrative ordinaire (budgets, marchés, personnel), exercée elle par le **gouvernement régional** via Bruxelles Pouvoirs Locaux. Cette dernière reconnaît elle-même que le contrôle linguistique « n'est pas inclus » dans sa tutelle et relève d'« un commissaire du gouvernement fédéral » — un **chevauchement de compétences** entre un contrôle d'origine fédérale et une tutelle régionale.

La chaîne complète illustre la complexité institutionnelle bruxelloise : le **vice-gouverneur** suspend → le **gouvernement régional** annule (ou non) → la **Commission permanente de contrôle linguistique (CPCL)**, organe fédéral consultatif, rend des avis non contraignants et peut saisir le **Conseil d'État**, seul à pouvoir annuler juridiquement. Plusieurs maillons, un résultat ténu.

## Un poste durable ?

Malgré ce bilan, **aucune proposition de suppression du vice-gouverneur n'a été recensée** dans les débats institutionnels récents. La sixième réforme de l'État a supprimé le gouverneur de Bruxelles, mais a **maintenu** le vice-gouverneur. Les seules propositions de réforme qui le visent directement vont, à l'inverse, dans le sens d'un **renforcement** : doter le vice-gouverneur d'un pouvoir d'**annulation** (et plus seulement de suspension), et faire débattre formellement son rapport annuel par le gouvernement régional.

## En quoi cela concerne le Bruxellois

La fonction touche un principe fondateur de la capitale : le **bilinguisme du service public**. Un Bruxellois doit pouvoir être accueilli et servi dans sa langue, française ou néerlandaise, par sa commune et son CPAS. Le vice-gouverneur est, sur le papier, le garant de ce droit.

Dans les faits, ses rapports documentent surtout l'**écart persistant** entre la règle et la pratique : une majorité d'agents recrutés sans le brevet linguistique requis, des suspensions sans suite, un service à 4 personnes. Le débat n'est donc pas tant celui du coût du poste — jamais chiffré — que celui de son **efficacité réelle** : un contrôle abondant mais sans pouvoir contraignant remplit-il sa mission, ou se limite-t-il à mesurer, chaque année, une non-conformité qu'il ne peut corriger ?

## À suivre

- **Rapport annuel** transmis au Parlement bruxellois : évolution du taux de conformité et des plaintes.
- **Débat sur les pouvoirs** : la question d'un pouvoir d'annulation (et non plus de simple suspension) revient régulièrement.
- **Numérisation** de la procédure de tutelle, demandée par le service depuis 2025.
- **Articulation** avec la réforme de l'administration régionale et la simplification institutionnelle bruxelloise.
