---
title: "Mobilité : nouveau plan régional, Métro 3 gelé, LEZ assouplie"
slug: mobility
locale: fr
domain: mobility
status: ongoing
blockedSince: "2024-06-09"
summary: "L'accord met fin au plan Good Move (remplacé par un nouveau Plan Régional de Mobilité), gèle le Métro 3 pour 10 ans au profit d'un tram, maintient la LEZ (pass 350 EUR/an, amende réduite à 80 EUR) et prévoit un 2e dimanche sans voiture. DPR officielle non encore publiée — mesures issues de sources de presse concordantes."
sectors:
  - transport
  - construction
sources:
  - label: "Bruxelles Mobilité — Projet Metro 3"
    url: "https://be.brussels/fr/transport-mobilite/parking-et-routes/projets/metro-3"
    accessedAt: "2026-02-06"
  - label: "Metro 3 — Site dédié du projet (STIB / Beliris)"
    url: "https://metro3.be/fr/project"
    accessedAt: "2026-02-06"
  - label: "STIB — Metro ligne 3 : travaux en cours"
    url: "https://www.stib-mivb.be/travel/works-and-projects/works-in-progress/metro-line-3"
    accessedAt: "2026-02-06"
  - label: "Good Move — Plan Régional de Mobilité (2020)"
    url: "https://be.brussels/fr/transport-mobilite/enjeux-de-la-mobilite/plan-regional-de-mobilite"
    accessedAt: "2026-02-06"
  - label: "Bruxelles Mobilité — Fermetures et rénovation des tunnels"
    url: "https://be.brussels/fr/transport-mobilite/parking-et-routes/chantiers/fermeture-de-tunnels"
    accessedAt: "2026-02-06"
  - label: "Parlement bruxellois — Recherche questions parlementaires"
    url: "https://www.parlement.brussels/weblex-quest-form/"
    accessedAt: "2026-02-06"
  - label: "Conseil d'État — Suspension permis Palais du Midi (18/12/2025)"
    url: "https://www.rtbf.be/article/apres-la-cour-des-comptes-le-conseil-d-etat-nouveau-coup-d-arret-pour-le-maudit-metro-3-11650687"
    accessedAt: "2026-02-08"
  - label: "ARAU — Victoire pour le Palais du Midi"
    url: "https://www.arau.org/fr/victoire-pour-le-palais-du-midi-le-permis-de-demolition-suspendu-par-le-conseil-detat/"
    accessedAt: "2026-02-08"
  - label: "DH — Tarifs STIB en hausse dès février 2026"
    url: "https://www.dhnet.be/regions/bruxelles/bruxelles-mobilite/2026/01/27/les-tarifs-de-la-stib-en-hausse-des-ce-dimanche-le-trajet-passe-a-240-labonnement-annuel-a-560-7CXAFM6DABB2VJMWCQUO4SUCBY/"
    accessedAt: "2026-02-08"
  - label: "Brussels Morning — Metro line 3: €4.6B project faces delays"
    url: "https://brusselsmorning.com/brussels-metro-line-3-update-e4-6b-project-faces-delays-elke-van-den-brandt-responds/58198/"
    accessedAt: "2026-02-10"
  - label: "BEI — Prêt de 475 millions EUR pour le métro Nord-Sud de Bruxelles"
    url: "https://www.eib.org/en/press/all/2023-546-brussels-gets-north-south-metro-line-with-backing-from-european-investment-bank"
    accessedAt: "2026-02-10"
  - label: "TomTom Traffic Index — Brussels (15e édition, données 2025)"
    url: "https://www.tomtom.com/traffic-index/brussels-traffic/"
    accessedAt: "2026-02-11"
  - label: "TomTom Traffic Index — Méthodologie"
    url: "https://www.tomtom.com/traffic-index/about/"
    accessedAt: "2026-02-11"
  - label: "Fleet.be — TomTom Traffic Index 2025 : la Belgique vue à travers ses embouteillages"
    url: "https://www.fleet.be/tomtom-traffic-index-2025-la-belgique-vue-a-travers-ses-embouteillages/?lang=fr"
    accessedAt: "2026-02-11"
  - label: "La Libre — Good Move : la « fin planifiée » au conclave (10 fév. 2026)"
    url: "https://www.lalibre.be/belgique/politique-belge/2026/02/10/elke-van-den-brandt-nest-pas-constructive-a-bruxelles-le-conclave-debute-sur-fond-de-tensions-avec-groen-7F7OQ4XO2NGAZLV5H5HBI7S5R4/"
    accessedAt: "2026-02-11"
  - label: "La Libre — LEZ : amendes de 350 EUR contestées, réduction proposée"
    url: "https://www.lalibre.be/economie/mes-finances/2026/02/10/totalement-disproportionnees-les-amendes-lez-pour-les-emissions-a-bruxelles-soulevent-lindignation-LAHDIYWVYRFF7JVTQWYZYO26YE/"
    accessedAt: "2026-02-11"
  - label: "La DH — Villo! concession expire en septembre 2026"
    url: "https://www.dhnet.be/regions/bruxelles/bruxelles-mobilite/2023/04/18/la-concession-de-villo-sarrete-en-2026-et-bruxelles-prepare-le-terrain-pour-lapres-voici-les-options-actuellement-sur-la-table-B6QQVO7BLRB7LBXVJILKQNSQDE/"
    accessedAt: "2026-02-11"
  - label: "DH — Sécurité, impôts, Good Move, logement : l'accord des 7 partis (12 fév. 2026)"
    url: "https://www.dhnet.be/regions/bruxelles/2026/02/12/securite-impots-good-move-logement-etc-voici-ce-que-contient-laccord-conclu-entre-les-sept-partis-du-nouveau-gouvernement-bruxellois-ZEAEQ3FBBJGS5BOSHK6632GJRI/"
    accessedAt: "2026-02-13"
  - label: "La Libre — Les principales mesures de l'accord (12 fév. 2026)"
    url: "https://www.lalibre.be/belgique/politique-belge/2026/02/12/nouveau-gouvernement-a-bruxelles-voici-les-principales-mesures-de-laccord-E2ZPXNRVKFASHGK6CTWDZ4BYTA/"
    accessedAt: "2026-02-13"
  - label: "RTBF — Ce que contient l'accord de gouvernement régional (12 fév. 2026)"
    url: "https://www.rtbf.be/article/equilibre-budgetaire-en-2029-good-move-primes-renolution-voici-ce-que-contient-l-accord-de-gouvernement-regional-a-bruxelles-11678417"
    accessedAt: "2026-02-13"
  - label: "VRT NWS — Dit weten we al over het Brusselse regeerakkoord (13 fév. 2026)"
    url: "https://www.vrt.be/vrtnws/nl/2026/02/13/brussels-regeerakkoord-overzicht-wat-weten-we-al/"
    accessedAt: "2026-02-13"
  - label: "BRUZZ — Ontdek de details uit het regeerakkoord (12 fév. 2026)"
    url: "https://www.bruzz.be/brusselkiest2024/brussel-kiest-brussels-parlement/ontdek-de-details-uit-het-regeerakkoord-van-pauze"
    accessedAt: "2026-02-13"
confidenceLevel: official
metrics:
  - label: "Metro 3 (nouvelles phases)"
    value: "Suspendu"
    source: "Bruxelles Mobilité"
    date: "2026-01-01"
  - label: "Metro 3 — Palais du Midi"
    value: "Permis suspendu"
    unit: "par le Conseil d'État (18/12/2025)"
    source: "Conseil d'État / ARAU"
    date: "2025-12-18"
  - label: "Tunnel Loi-Belliard"
    value: "161"
    unit: "millions EUR gelés (permis obtenus)"
    source: "Bruxelles Mobilité / Parlement bruxellois"
    date: "2026-01-01"
  - label: "Metro 3 (coût total estimé)"
    value: "4,6"
    unit: "milliards EUR"
    source: "Brussels Morning / Parlement bruxellois"
    date: "2026-02-10"
  - label: "Metro 3 (dépassement budgétaire)"
    value: "+255%"
    unit: "vs budget initial, horizon 2034+"
    source: "Commission spéciale Parlement bruxellois"
    date: "2025-12-15"
  - label: "Metro 3 — Prêt BEI"
    value: "475"
    unit: "millions EUR (conversion prémétro Albert–Gare du Nord)"
    source: "Banque européenne d'investissement"
    date: "2023-12-21"
  - label: "STIB — Hausse tarifaire"
    value: "+1,23%"
    unit: "au 01/02/2026 (ticket : 2,40 €, abonnement annuel : 560 €)"
    source: "STIB / contrat de gestion 2024-2028"
    date: "2026-02-01"
  - label: "Good Move (nouvelles phases)"
    value: "Gelé"
    source: "Bruxelles Mobilité"
    date: "2026-01-01"
  - label: "Congestion — heures perdues/an"
    value: "146"
    unit: "heures (+6h23 vs 2024, +42h vs 2023)"
    source: "TomTom Traffic Index (15e éd., données 2025)"
    date: "2026-01-21"
  - label: "Congestion — vitesse moyenne"
    value: "18,3"
    unit: "km/h (la plus basse des villes belges)"
    source: "TomTom Traffic Index (15e éd., données 2025)"
    date: "2026-01-21"
  - label: "Good Move au conclave"
    value: "Fin planifiée"
    unit: "annonce MR/Engagés au conclave (10/02/2026)"
    source: "La Libre / FLEET"
    date: "2026-02-10"
  - label: "LEZ — amendes contestées"
    value: "350"
    unit: "EUR, réduction proposée au conclave"
    source: "La Libre / L'Avenir"
    date: "2026-02-09"
  - label: "Villo! concession"
    value: "Expire sept. 2026"
    unit: "pas de successeur identifié"
    source: "La DH / Bruxelles Today"
    date: "2026-09-30"
  - label: "Accord — Good Move"
    value: "Remplacé"
    unit: "par un nouveau Plan Régional de Mobilité"
    source: "RTBF / La Libre (fév. 2026)"
    date: "2026-02-12"
  - label: "Accord — Métro 3"
    value: "Gelé 10 ans"
    unit: "remplacé par un tram (boucle)"
    source: "DH / RTBF (fév. 2026)"
    date: "2026-02-12"
  - label: "Accord — LEZ"
    value: "Pass 350 EUR/an"
    unit: "amende réduite à 80 EUR, plafond annuel supprimé"
    source: "DH / VRT NWS / BRUZZ (fév. 2026)"
    date: "2026-02-12"
  - label: "Accord — Dimanche sans voiture"
    value: "2"
    unit: "par an (vs 1 actuellement)"
    source: "RTBF (fév. 2026)"
    date: "2026-02-12"
  - label: "Accord — Tram 15"
    value: "Maintenu"
    unit: "Gare du Nord – Tour & Taxis"
    source: "DH / RTBF (fév. 2026)"
    date: "2026-02-12"
  - label: "Accord — Bollards contestés"
    value: "Retrait"
    source: "DH / La Libre (fév. 2026)"
    date: "2026-02-12"
lastModified: "2026-02-13"
changeType: status-change
changeSummary: "Accord de gouvernement : statut delayed → ongoing, Good Move remplacé, Métro 3 gelé 10 ans, LEZ assouplie (pass 350 EUR, amende 80 EUR)"
summaryFalc: "Le plan Good Move est remplacé par un nouveau plan de mobilité. Le Métro 3 est gelé pour 10 ans. La zone de basses émissions reste mais les amendes sont réduites."
---

## Accord de gouvernement : les engagements annoncés

> **Note** : Le texte officiel de la DPR n'a pas encore été publié. Les mesures ci-dessous sont issues de sources de presse concordantes (RTBF, La Libre, DH, BX1, VRT NWS, BRUZZ). Elles seront mises à jour dès la publication du document officiel.

L'accord du 12 février 2026 opère un virage majeur en matière de mobilité :

- **Good Move** : fin du nom et du plan tel quel, remplacé par un **nouveau Plan Régional de Mobilité** avec des mailles plus petites (périmètres d'écoles)
- **Métro 3** : **gelé pour 10 ans**, remplacé par un tram complétant la boucle du réseau existant
- **Tram 15** (Gare du Nord – Tour & Taxis) : maintenu
- **Chantiers STIB** : Constitution et Palais du Midi poursuivis
- **Dimanche sans voiture** : un deuxième dimanche par an
- **LEZ** : maintenue — **pass annuel à 350 EUR** (tarif social 200 EUR), amende par infraction réduite de 350 à **80 EUR**, plafond de 4 amendes/an supprimé
- **Tarifs STIB** : préférentiels jeunes/seniors maintenus
- **Bollards contestés** : retrait

Le gel du Métro 3 pour 10 ans est la décision la plus structurante : il met fin au projet tel qu'il était conçu (extension vers Bordet, 4,6 milliards EUR) et réoriente l'investissement vers le tramway.

Les sections suivantes documentent la situation héritée de la crise politique (juin 2024 – février 2026).

## Ce qui était bloqué

### Metro 3 — Ligne Nord-Sud

Le projet Metro 3 vise à créer une nouvelle ligne de métro reliant le nord de Bruxelles (Bordet) au sud (Albert). Le coût total du projet est désormais estimé à **4,6 milliards EUR**. Les premières phases (pré-métro existant vers métro complet entre Albert et Gare du Nord) sont en cours, soutenues par un **prêt de 475 millions EUR de la Banque européenne d'investissement** (BEI).

En revanche, une **commission spéciale du Parlement bruxellois** (fin 2025) a révélé un **dépassement budgétaire de 255 %** par rapport au budget initial, avec un horizon de livraison repoussé à **2034 au plus tôt**.

De plus, le **Conseil d'État a suspendu le permis d'urbanisme** autorisant la déconstruction-reconstruction du Palais du Midi (arrêt du 18 décembre 2025). La Région avait violé les règles de protection du patrimoine en accordant le permis avant l'évaluation patrimoniale. Les travaux prévus pour février 2026 ne peuvent pas démarrer. Le dossier est porté par l'ARAU et Inter-Environnement Bruxelles.

L'extension vers Bordet (la partie la plus ambitieuse) nécessite des décisions politiques nouvelles :

- Réévaluation du budget global et de la soutenabilité financière
- Approbation du tracé définitif au nord de Schaerbeek
- Coordination avec les Plans d'Aménagement Directeur (PAD)

Sans gouvernement de plein exercice, ces décisions ne peuvent pas être prises.

### Tunnels routiers — le cas Loi-Belliard

Bruxelles compte plus de 30 tunnels routiers, dont plusieurs nécessitent des rénovations majeures. Le cas le plus emblématique est le **tunnel Loi-Belliard** : les permis d'urbanisme ont été obtenus, le budget de **161 millions EUR** est identifié, mais le chantier ne peut pas démarrer en l'absence d'un gouvernement de plein exercice pour engager la dépense.

Les tunnels Stéphanie et Bailli figurent également parmi les priorités. La maintenance courante continue (assurée par l'administration), mais les grands projets de rénovation structurelle nécessitent :

- Un engagement budgétaire pluriannuel (enveloppes de plusieurs dizaines de millions d'euros par tunnel)
- Un choix politique sur les priorités
- Souvent, une coordination avec d'autres projets (Good Move, aménagements cyclables)

### Good Move — Plan régional de mobilité

Good Move est le plan régional de mobilité adopté en 2020. Les mailles déjà mises en place continuent de fonctionner. Cependant, **le déploiement des nouvelles mailles et les ajustements basés sur les retours citoyens** nécessitent des décisions gouvernementales. En affaires courantes, seuls les ajustements mineurs sont possibles.

Au conclave de formation (10 février 2026), le MR et Les Engagés ont annoncé leur intention de mettre **fin au plan Good Move** sous sa forme actuelle. La mesure est contestée par Groen, qui défend le plan. Le site anti-Good Move lancé par le MR (7 février 2026) illustre la polarisation politique autour du sujet. L'avenir de Good Move dépend directement de l'issue des négociations de formation.

## Ce qui continue de fonctionner

- **STIB** : les opérations quotidiennes (bus, tram, métro) continuent normalement sous le contrat de gestion 2024-2028. Les tarifs ont augmenté de 1,23 % au 1er février 2026 (ticket : 2,40 €, abonnement annuel : 560 €)
- **Villo** : service de vélos partagés sous concession existante, mais la **concession expire en septembre 2026** sans successeur identifié. Le remplacement du système nécessite une décision gouvernementale
- **Jump/Lime** : les services de trottinettes et vélos électriques sous concession
- **Maintenance routière courante** : nids-de-poule, signalisation, feux de circulation
- **Travaux déjà approuvés** : les chantiers lancés avant juin 2024 se poursuivent

### LEZ — Zone de basses émissions

La zone de basses émissions (LEZ) de Bruxelles impose des amendes de **350 EUR** aux véhicules non conformes. Ce montant est contesté par plusieurs partis au conclave : une réduction du montant est proposée, tout en maintenant le principe de la zone. Le Conseil d'État doit rendre un avis fin février 2026 sur la légalité du barème actuel.

## Congestion routière : les données TomTom 2025

Selon le TomTom Traffic Index (15e édition, données 2025), un trajet de 10 km à Bruxelles prend en moyenne **32 minutes 47 secondes**, soit 21 secondes de plus qu'en 2024. Les automobilistes bruxellois perdent en moyenne **146 heures par an** dans les embouteillages — 6 heures 23 minutes de plus qu'en 2024, et 42 heures de plus qu'en 2023.

Le niveau de congestion (temps additionnel par rapport au flux libre) s'établit à **46,2 %** (+1,3 point). Aux heures de pointe du soir, ce chiffre atteint 90,5 %.

### Changement de méthodologie

Le classement de Bruxelles varie fortement selon la métrique utilisée. Classée par temps de trajet absolu, Bruxelles figure au **15e rang mondial**. Classée par pourcentage de congestion, elle recule au-delà du 60e rang européen. Cette différence s'explique par un triple changement dans l'édition 2025 : nouvelle définition des périmètres urbains (grilles hexagonales), passage du classement principal au pourcentage de congestion, et élargissement du panel à environ 500 villes.

Les comparaisons avec les classements des années précédentes (où Bruxelles figurait dans le « top 10 mondial ») doivent être interprétées avec prudence : elles ne reflètent pas une amélioration du trafic, mais un changement d'instrument de mesure.

La vitesse moyenne à Bruxelles reste la plus basse des villes belges (18,3 km/h), en partie liée à la zone 30 généralisée, à la densité du tissu urbain et au réseau de feux de signalisation.

## Pourquoi c'est important

La mobilité est le domaine le plus visible pour les citoyens bruxellois au quotidien. Chaque jour de retard dans les projets d'infrastructure a un coût :

- Coût économique (congestion, temps perdu)
- Coût environnemental (retard dans la transition vers les transports publics)
- Coût social (accessibilité des quartiers mal desservis)

Les données précises sur ces coûts sont difficiles à estimer car elles dépendent de modélisations complexes. BGM ne publie pas d'estimation chiffrée du coût économique de ces retards pour éviter toute spéculation non sourcée.
