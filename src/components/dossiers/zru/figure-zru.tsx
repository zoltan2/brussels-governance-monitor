 ...
 */
const L: Record<Locale, { source: string; extrait: string; confiance: string; donnees: string; modifs: string }> = {
  fr: { source: 'Source', extrait: 'données extraites le', confiance: 'confiance', donnees: 'Voir les données', modifs: 'Traitements BGM' },
  nl: { source: 'Bron', extrait: 'gegevens opgehaald op', confiance: 'betrouwbaarheid', donnees: 'Gegevens bekijken', modifs: 'Bewerkingen door BGM' },
  en: { source: 'Source', extrait: 'data extracted on', confiance: 'confidence', donnees: 'View the data', modifs: 'BGM processing' },
  de: { source: 'Quelle', extrait: 'Daten abgerufen am', confiance: 'Vertrauen', donnees: 'Daten anzeigen', modifs: 'Bearbeitung durch BGM' },
};
 ...
 * texte incrusté, légende HTML (indicateur, période, source, date
 * d'extraction, confiance) et tableau de données équivalent replié dans un
  ...
  return (
    <figure aria-labelledby={idTitre} className="my-8 rounded-lg border border-neutral-200 bg-neutral-50 p-4">
      ...
      <figcaption className="mt-3 text-xs leading-relaxed text-neutral-600">
        ...
        {l.source} :{' '}
        <a href={p.provenance.url} className="text-brand-700 underline [overflow-wrap:anywhere] hover:text-brand-900">
          {p.provenance.producteur}
        </a>
        , {l.extrait} <time dateTime={p.provenance.extraitLe}>{p.provenance.extraitLe}</time> ; {l.confiance} :{' '}
