  ...
  return (
    <section className="bg-gradient-to-b from-slate-800 to-slate-700 py-16 text-white">
      <div className="mx-auto max-w-5xl px-4 text-center">
        ...
        <p className="mt-1 text-sm font-medium text-white/85">{t('closed')}</p>
        ...
        <div className="mx-auto mt-8 max-w-lg rounded-lg border border-white/10 bg-white/5 px-5 py-4 text-left">
          <div className="flex items-start gap-3">
            <Shield size={18} className="mt-0.5 shrink-0 text-white/75" aria-hidden={true} />
            <div>
              <p className="text-sm font-medium text-white/90">{th('identity')}</p>
              <p className="mt-0.5 text-xs text-white/80">{th('identityDetail')}</p>
              <Link
                href="/about"
              ...
              >
                {th('identityLink')}
                <ArrowRight size={12} className="text-white/80" aria-hidden={true} />
              </Link>
            </div>
          </div>
        </div>
